...
class Reflector implements ReflectorInterface
{
    ...
    private function createParameterEntity(\ReflectionParameter $reflection, $docs)
    {
        ...
        if ($declaredType && !($declaredType == 'array' && substr($docs['type'], -2) == '[]')
            && $declaredType != $docs['type']
        ) {
        ...
        if (!empty($docs)) {
            ...
            if ($type && $def && !empty($docs['type'])
                && $docs['type'] != $type && strpos($docs['type'], '|') === false
            ) {
        ...
        $param->setType(empty($docs['type'])
            ? 'mixed'
            : str_replace(array('|', '\\\\'), array('/', '\\'), $docs['type']));
        return $param;
    }
    ...
    public static function getPropertyType(array $tags)
    {
    ...
    public static function getPropertyDescription(array $tags)
    {
        if (isset($tags['var']) && $tags['var']) {
            $words = explode(' ', trim($tags['var']));
            array_shift($words);
    ...
    {
        $ns = $reflection instanceof \ReflectionClass
        ...
        foreach ($lines as $line) {
             ...
             *
             * This removes unneeded spaces in lines.
             */
            $words = array_values(array_filter($words));

            if ($current_tag !== 'description' && empty($words)) {
                continue;
            }
            ...
            if (empty($words) || strpos($words[0], '@') === false) {
                 ...
                 * Append to tag
                 *
                 * Keep line breaks in examples and descriptions
                 */
                $joinWith = in_array($current_tag, [ 'example', 'description' ], true)
                    ? PHP_EOL
                    : ' ';
    ...
    {
        $natives = array('mixed', 'string', 'int', 'number', 'bool', 'boolean',
            'object', 'mixed', 'false', 'true', 'null', 'array', 'void'
        );

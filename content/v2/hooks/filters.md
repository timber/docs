...
## timber/get\_posts/mirror\_wp\_get\_posts

Filters whether Timber::get_posts() should mirror WordPress’s get_posts() function.

**DEPRECATED** since 2.0.0 use the desired args within Timber\PostQuery() instead
...
| Name | Type | Description |
| --- | --- | --- |
| $mirror | `bool` | Whether to mirror the `get_posts()` function of WordPress with all its parameters. Default `false`. |
...
## timber\_post\_getter\_get\_posts
...
| Name | Type | Description |
| --- | --- | --- |
| $post_class | `string` or `array` | The post class(es) to use. Can be a string for a single post class or an key-value array map to define which post type should use which class. Default `Timber\Post`. |
...
**DEPRECATED** since 2.0.0, use `timber/post/post_class`
